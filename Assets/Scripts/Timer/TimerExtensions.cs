...
public static class TimerExtensions
{
    ...
    /// <param name="onTimerInvoke">The action to run when the timer elapses.</param>
    ...
    public static void AttachTimer(this MonoBehaviour behaviour,
                                   float duration,
                                   Action<float, object> onTimerInvoke,
                                   bool isLooped = false,
                                   bool useRealTime = false)
    {
        Timer.Register(duration, onTimerInvoke, isLooped, useRealTime, behaviour);
    }
}
